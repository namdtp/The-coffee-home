...
class ProductsFilter extends Model
{
    ...
    public static function getSizes($url){
        ...
        $getProductSizes = ProductsAttribute::select('size')->whereIn('product_id', $getProductIds)->groupBy('size')
        ->pluck('size')->toArray();
        // echo "<pre>";
    ...
    public static function getColors($url){
        $categoryDetails = Category::categoryDetails($url);
        $getProductIds = Product::select('id')->whereIn('category_id', $categoryDetails['catIds'])->pluck('id')->toArray();
        $getProductColors = Product::select('product_type')->whereIn('id', $getProductIds)->groupBy('product_type')->pluck('product_type')->toArray();
