<div class="form-group">    
    ...
    <select id="parent_id" name="parent_id" class="form-control" style="color: #000;">
        ...
        @if(!empty($getCategories))
            @foreach($getCategories as $parentcategory)
                ...
                selected="" @endif>{{ $parentcategory['category_name'] }}
                </option>
                @if(!empty($parentcategory['subcategories']))
                    @foreach($parentcategory['subcategories'] as $subcategory)
                <option value="{{ $subcategory['id'] }}" @if(isset($subcategory['parent_id']) && $subcategory['parent_id'] == $subcategory['id']) selected="" 
                @endif>&nbsp;&raquo;&nbsp;{{ $subcategory['category_name'] }}
                </option>     
                @endforeach
                @endif       
            @endforeach
